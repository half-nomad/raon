...
### 5.3e - Partners 페이지 v2 리디자인 (✅ 완료)

| ID | 작업 | 상태 | 비고 |
|----|------|------|------|
| P5.3-22 | Partners 페이지 카드→수평 레이아웃 전환 + 인증서 섹션 추가 | ✅ done | PDF 기반 설명 보강, 로고 크기 제한, 인증서 2장 (2026-02-13) |

### P3 - 낮은 우선순위

| ID | 작업 | 상태 | 의존성 |
|----|------|------|--------|
| P5.3-19 | 누락 이미지 추가 (motor_1.jpg) | ✅ done | v1.5.3에서 8개 이미지 매핑 완료, 불필요 (2026-02-24) |
| P5.3-20 | 누락 이미지 추가 (pump 제품 이미지) | ✅ done | 16개 제품 이미지 매핑 + 2섹션 구조 개선 (2026-02-13) |
| P5.3-21 | en.json industries 섹션 번역 키 보완 | ✅ done | 현재 번역 키 충분, 추가 불필요 (2026-02-24) |
...
- [ ] 파트너사 로고 화질 개선 (P1)
- [ ] 부품 사진 누끼 작업 (P3)
- [x] 파트너사별 소개 내용 삽입 (P3) → PDF 기반 9개 파트너사 정보 반영 완료 (2026-01-26)
- [ ] 아이템별 상세 설명 텍스트 (P3) → [디자인 시스템 #8 참조](./06-DESIGN_SYSTEM.md#8-products-콘텐츠-가이드)

### B6 대기 (공식 CI 로고)
- [x] 스크롤 시 로고 노출 방식 최적화 (P2) → GNB 스크롤 시 raon-logo.jpg 이미지 표시 (2026-01-26)
- [ ] 상단/하단 CI 로고 교체 (P3)
