...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const industryT = await getTranslations({ locale, namespace: 'industries.powerPlant' });
  ...
  return {
    ...
    openGraph: {
      ...
      description: industryT('description'),
      images: ['/images/og/power-plant-og.jpg'],
...
export default async function PowerPlantPage() {
  const t = await getTranslations("industries.powerPlant");
    ...
    <div className="min-h-screen">
      <BreadcrumbSchema
        items={[
          ...
          { name: t("title"), url: '/industries/power-plant' },
        ]}
      />
      <Header />
      ...
      <section className="py-12 bg-slate-50">
        <div className="section-container">
          <div className="relative w-full h-[300px] sm:h-[400px] lg:h-[500px] rounded-2xl overflow-hidden shadow-xl">
            <Image
              src="/images/industries/power-plant.jpg"
              alt={t("title")}
              fill
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {relatedProducts.map((product, index) => (
                <Link
                  key={index}
                  href={product.href}
                  className="flex items-center justify-center px-4 py-4 min-h-11 bg-white border-2 border-gray-200 rounded-full text-xs sm:text-sm font-semibold text-gray-700 hover:border-[#3B82F6] hover:text-[#3B82F6] hover:shadow-lg transition-all"
                >
                  {product.name}
                </Link>
              ))}
